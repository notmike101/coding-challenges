...
const twoSum = (nums: number[], target: number): number[] => {
  const usedNumberIndexes: number[] = [];

  for (let index = 0; index < nums.length; ++index) {
    const searchForTargetIndex = nums.indexOf(target - nums[index]);

    if (searchForTargetIndex !== -1 && searchForTargetIndex !== index) {
      usedNumberIndexes.push(index, searchForTargetIndex);
      break;
    }
  }

  return usedNumberIndexes;
};
